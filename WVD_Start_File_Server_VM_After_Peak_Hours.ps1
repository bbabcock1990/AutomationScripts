<#
.SYNOPSIS
    Automated process of starting WVD File Server after peak-hours.
.DESCRIPTION
    This script is intended to automatically start the Windows Virtual Desktop File Server
    before peak-hours. The script pulls the WVD File Server from Azure and runs the Start-AzVM 
...
try{
    ...
    Start-AzVM -ErrorAction Stop -ResourceGroupName $fileServerRG -Name $fileServerVMName -AsJob
    
}
catch{
    $ErrorMessage = $_.Exception.message
    Write-Error ("Error starting down File Server: " + $ErrorMessage)
    Break
}
